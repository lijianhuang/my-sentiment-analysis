...
parse_report.py
---------------
Auto-detects the latest UOB_Malaysia_BUZZ_*.txt in a folder, calls OpenAI
to extract structured data, validates with Pydantic, writes JSON.

Usage:
    python parse_report.py                        # looks in current dir
    python parse_report.py --folder ./reports     # looks in ./reports
    python parse_report.py --input report.txt     # explicit file override
    python parse_report.py --output out.json      # explicit output path
"""
...
FILE_PATTERN = re.compile(
    r"UOB_Malaysia_BUZZ_(\d{4}-\d{2}-\d{2})_(\d{4})\.txt$",
    re.IGNORECASE
)

def find_latest(folder: Path) -> Path:
    """Return the most recent UOB_Malaysia_BUZZ_*.txt in folder by filename timestamp."""
    candidates = []
    for f in folder.glob("UOB_Malaysia_BUZZ_*.txt"):
    ...
    if not candidates:
        print(f"No files matching UOB_Malaysia_BUZZ_YYYY-MM-DD_HHMM.txt found in: {folder}")
        sys.exit(1)
